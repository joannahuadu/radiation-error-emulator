...
To further quantify radiation-caused SEUs/MCUs' impacts on diverse in-orbit applications at scale, a flexible and reconfigurable error emulator is desirable to facilitate both the exploration of applications’ error behaviors and the availability of application-aware tolerant design.
...
#include "mem_utils.h"
MemUtils memUtils;
// Option 1: Inject errors in the global area, Vaddr is the virtual address in the process space (i.e., the start address of ROI), and size is the ROI's size. Set the start (e.g., 300B for ResNet-50) and end bytes (e.g., 500000B for ResNet-50) to be omitted, because these bytes can easily cause inference to abort.
